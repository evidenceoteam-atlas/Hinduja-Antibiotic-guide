...
| Legacy mobile authentication assertions no longer matched the application. | Federated sign-in support and its static test were removed; `tests/test_mobile_unauthenticated_access_static.py` now asserts direct access, local-only profile storage, and absence of callback/session code. |
| Web preview blank in browser despite a clean bundle. The Supabase public variables were absent, so clinical data requests used placeholder values. | Populate `EXPO_PUBLIC_SUPABASE_URL` and `EXPO_PUBLIC_SUPABASE_ANON_KEY` for approved clinical-content reads and restart Expo. These variables no longer configure authentication. |
...
| Three overlapping audit docs reported the same gaps under different dates. | Consolidated into this ledger; obsolete authentication setup documents were subsequently removed with the mobile authentication flow. |
...
| Expo dependency versions must continue to match SDK 54. `@react-native-async-storage/async-storage` is used for local profile personalization; secure token storage is no longer required by the mobile app. |
...
| Finding | Reason |
|---|---|
| Demo OTP logged in cleartext at `services/auth-service/app/main.py:51`. | The auth-service is not on the mobile path. Fix before this separate scaffold goes live. |
...
| The backend OTP/JWT auth service remains a disconnected scaffold. | It is not a mobile authentication method; finish or remove it separately if backend clients begin using it. |
